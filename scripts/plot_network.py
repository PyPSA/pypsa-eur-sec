            ...
            (costs,
             (getattr(
                 ...
                 (getattr(
                  ...
                  getattr(
                     n,
                     comp).nice_group)).sum().unstack().fillna(0.)),
            axis=1)
...
def plot_series(carrier="AC"):
    ...
    new_columns = (
        preferred_order & supply.columns).append(
        supply.columns.difference(preferred_order))

    supply.loc[start:stop, new_columns].plot(ax=ax, kind="area", stacked=True, linewidth=0., color=[
                                             snakemake.config['plotting']['tech_colors'][i.replace(suffix, "")] for i in new_columns])
...
if __name__ == "__main__":
    ...
    if 'snakemake' not in globals():
        ...
        snakemake.input.network = "{}{}/postnetworks/elec_s_181_{}__Co2L0-3H-T-H-B-I-solar3.nc".format(
            snakemake.config['results_dir'], snakemake.config['run'], snakemake.input.scenario)
        snakemake.output.network = "{}{}/maps/elec_s_181_{}__Co2L0-3H-T-H-B-I-solar3.pdf".format(
            snakemake.config['results_dir'], snakemake.config['run'], snakemake.input.scenario)
