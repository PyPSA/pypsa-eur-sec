...
def build_biomass_potentials():
    ...
    for i in range(36):
        df_dict[df.iloc[i * 16, 1]] = df.iloc[1 + \
            i * 16:(i + 1) * 16].astype(float)
    ...
    biomass_potentials = df_new.loc[idx[:,
                                        snakemake.config['biomass']['year'],
                                        snakemake.config['biomass']['scenario']],
                                    :].groupby(us_type,
                                               axis=1).sum().groupby(level=0).sum().rename({"UK": "GB",
                                                                                            "BH": "BA"}) / 3.6 * 1e6
