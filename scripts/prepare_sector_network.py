...
def prepare_data(network):
    ...
    traffic = pd.read_csv(
        "/home/ws/bw0928/Dokumente/pypsa-eur-sec/data/emobility/KFZ__count",
        skiprows=2)["count"]
    ...
    traffic = pd.read_csv(
        "/home/ws/bw0928/Dokumente/pypsa-eur-sec/data/emobility/Pkw__count",
        skiprows=2)["count"]
...
def add_storage(network):
    ...
    for candidate in candidates.index:
        buses = sorted([candidates.at[candidate, "bus0"],
...
def add_heat(network):
    ...
    for sector in sectors:
        ...
        if options["central"]:
            ...
            nodes[sector +
                  " urban decentral"] = pop_layout.index[pop_layout.ct.isin(urban_decentral_ct)]
        else:
        ...
        if options["chp"]:
            ...
            if name == "urban central":
                ...
                network.madd("Link",
                             ...
                             p_nom_extendable=True,
                             capital_cost=costs.at['central gas CHP',
                                                   'fixed'] * costs.at['central gas CHP',
                                                                       'efficiency'],
                             ...
                             efficiency2=costs.at['gas',
                                                  'CO2 intensity'],
                             c_b=costs.at['central gas CHP',
                                          'c_b'],
                             c_v=costs.at['central gas CHP',
                                          'c_v'],
                             p_nom_ratio=costs.at['central gas CHP',
                                                  'p_nom_ratio'])
                ...
                network.madd("Link",
                             ...
                             p_nom_extendable=True,
                             capital_cost=costs.at['central gas CHP CCS',
                                                   'fixed'] * costs.at['central gas CHP CCS',
                                                                       'efficiency'],
                             ...
                             efficiency3=costs.at['gas',
                                                  'CO2 intensity'] * options["ccs_fraction"],
                             c_b=costs.at['central gas CHP CCS',
                                          'c_b'],
                             c_v=costs.at['central gas CHP CCS',
                                          'c_v'],
                             p_nom_ratio=costs.at['central gas CHP CCS',
                                                  'p_nom_ratio'])
    ...
    if options['retrofitting']:
        ...
        # HPI 3.4e9m^2 for DE res, 2.3e9m^2 for tert
        # https://doi.org/10.1016/j.rser.2013.09.012
        square_metres = population[retro_nodes] / population['DE'] * 5.7e9
        ...
        network.madd('Generator',
                     ...
                     p_max_pu=space_pu,
                     p_min_pu=space_pu,
                     capital_cost=options['retrofitting-cost_factor'] * costs.at['retrofitting I',
                                                                                 'fixed'] * square_metres / (options['retroI-fraction'] * space_peak))
        ...
        network.madd('Generator',
                     ...
                     p_max_pu=space_pu,
                     p_min_pu=space_pu,
                     capital_cost=options['retrofitting-cost_factor'] * costs.at['retrofitting II',
                                                                                 'fixed'] * square_metres / (options['retroII-fraction'] * space_peak))
...
def add_biomass(network):
    ...
    if not urban_central.empty and options["chp"]:
        ...
        network.madd("Link",
                     ...
                     p_nom_extendable=True,
                     capital_cost=costs.at['central solid biomass CHP',
                                           'fixed'] * costs.at['central solid biomass CHP',
                                                               'efficiency'],
                     marginal_cost=costs.at['central solid biomass CHP',
                                            'VOM'],
                     efficiency=costs.at['central solid biomass CHP',
                                         'efficiency'],
                     c_b=costs.at['central solid biomass CHP',
                                  'c_b'],
                     c_v=costs.at['central solid biomass CHP',
                                  'c_v'],
                     p_nom_ratio=costs.at['central solid biomass CHP',
                                          'p_nom_ratio'])
        ...
        network.madd("Link",
                     ...
                     p_nom_extendable=True,
                     capital_cost=costs.at['central solid biomass CHP CCS',
                                           'fixed'] * costs.at['central solid biomass CHP CCS',
                                                               'efficiency'],
...
def add_industry(network):
    ...
    network.madd("Link",
                 ...
                 p_nom_extendable=True,
                 capital_cost=costs.at["industry CCS",
                                       "fixed"] * costs.at['solid biomass',
                                                           'CO2 intensity'] * 8760,
                 # 8760 converts EUR/(tCO2/a) to EUR/(tCO2/h)
                 efficiency=0.9,
                 efficiency2=-costs.at['solid biomass',
                                       'CO2 intensity'] * options["ccs_fraction"],
    ...
    network.madd("Load",
                 ...
                 p_set=nodal_energy_totals.loc[nodes,
                                               ["total international aviation",
                                                "total domestic aviation"]].sum(axis=1).sum() * 1e6 / 8760.)
    ...
    # tco2 per hour
    co2 = network.loads.loc[["naphtha for industry",
                             "kerosene for aviation"],
                            "p_set"].sum() * costs.at["oil",
                                                      'CO2 intensity'] - industrial_demand.loc[nodes,
    ...
    network.madd("Link",
                 ...
                 p_nom_extendable=True,
                 # 8760 converts EUR/(tCO2/a) to EUR/(tCO2/h)
                 capital_cost=costs.at["industry CCS", "fixed"] * 8760,
