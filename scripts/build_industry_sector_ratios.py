...
excel_out = pd.read_excel(
    '{}/JRC-IDEES-2015_Industry_{}.xlsx'.format(
        base_dir,
        country),
    sheet_name=sub_sheet_name_dict[sector],
    index_col=0,
    header=0,
    squeeze=True)  # the summary sheet
...
excel_fec = pd.read_excel(
    ...
    index_col=0,
    header=0,
    squeeze=True)  # the final energy consumption sheet
...
excel_ued = pd.read_excel(
    ...
    index_col=0,
    header=0,
    squeeze=True)  # the used energy sheet
...
excel_emi = pd.read_excel(
    ...
    index_col=0,
    header=0,
    squeeze=True)  # the emission sheet
...
eff_met = s_ued['Natural gas (incl. biogas)'] / \
...
eff = s_ued['Steel: Furnaces, Refining and Rolling - Electric'] / \
...
# this process can be electrified
eff = s_ued['Steel: Products finishing - Electric'] / \
...
df.loc['process emission', sector] = s_emi['Process emissions'] / \
    s_out[sector]  # unit tCO2/t material

# final energy consumption per t
df.loc[['elec', 'heat', 'methane'], sector] = df.loc[['elec', 'heat',
                                                      'methane'], sector] * conv_factor / s_out[sector]  # unit MWh/t material
...
df.loc['hydrogen', 'Integrated steelworks'] = snakemake.config["industry"]["H2_DRI"] * \
    snakemake.config["industry"]["DRI_ratio"]
...
df.loc['methane', sector] += s_fec['Natural gas'] - \
    snakemake.config["industry"]["H2_for_NH3"] / conv_factor
df.loc['hydrogen', sector] += snakemake.config["industry"]["H2_for_NH3"] / conv_factor
# 1 ktoe = 11630 MWh

# LPG and other feedstock materials are assimilated to naphtha since they
# will be produced trough Fischer-Tropsh process
df.loc['naphtha', sector] += (s_fec['Solids'] +
                              s_fec['Refinery gas'] +
                              s_fec['LPG'] +
                              s_fec['Diesel oil'] +
...
eff_elec = s_ued['Chemicals: Furnaces - Electric'] / \
    s_fec['Chemicals: Furnaces - Electric']
...
eff_elec = s_ued['Chemicals: Process cooling - Electric'] / \
...
# these are emissions originating from feedstock, i.e. could be non-fossil
# origin
df.loc['process emission from feedstock', sector] += (
    snakemake.config["industry"]['petrochemical_process_emissions'] * 1e3) / s_out.values  # unit tCO2/t material


# final energy consumption per t
sources = ['elec', 'biomass', 'methane', 'hydrogen', 'heat', 'naphtha']

df.loc[sources, sector] = df.loc[sources, sector] * \
    conv_factor / s_out.values  # unit MWh/t material
# 1 ktoe = 11630 MWh
...
eff_elec = s_ued['High enthalpy heat  processing - Electric (microwave)'] / \
...
eff = s_ued['Chemicals: Process cooling - Electric'] / \
    s_fec['Chemicals: Process cooling - Electric']
...
df.loc[sources, sector] = df.loc[sources, sector] * \
    11.630 / s_out.values  # unit MWh/t material
# 1 ktoe = 11630 MWh
...
eff = s_ued['Chemicals: Furnaces - Electric'] / \
    s_fec['Chemicals: Furnaces - Electric']
...
df.loc['biomass',
       sector] += s_ued[['Cement: Grinding, milling of raw material',
                         'Cement: Pre-heating and pre-calcination']].sum() / eff_bio
...
df.loc['biomass',
       sector] += s_ued[['Cement: Clinker production (kilns)',
                         'Cement: Grinding, packaging']].sum() / eff_bio
...
eff_elec = s_ued['Ceramics: Microwave drying and sintering'] / \
    s_fec['Ceramics: Microwave drying and sintering']
df.loc['elec', sector] += s_ued[['Ceramics: Mixing of raw material',
                                 'Ceramics: Drying and sintering of raw material']].sum() / eff_elec
...
eff_elec = s_ued['Ceramics: Electric furnace'] / \
    s_fec['Ceramics: Electric furnace']
...
# Efficiency changes due to electrification
eff_elec = s_ued['Glass: Electric melting tank'] / \
    s_fec['Glass: Electric melting tank']
df.loc['elec', sector] += s_ued['Glass: Melting tank'] / eff_elec

eff_elec = s_ued['Glass: Annealing - electric'] / \
    s_fec['Glass: Annealing - electric']
df.loc['elec', sector] += s_ued[['Glass: Forming', 'Glass: Annealing',
                                 'Glass: Finishing processes']].sum() / eff_elec
...
# Industry-specific
df.loc['elec',
       sector] += s_fec[['Pulp: Wood preparation, grinding',
                         'Pulp: Cleaning']].sum()
...
df.loc[sources, sector] = df.loc[sources, sector] * conv_factor / \
    s_out['Pulp production (kt)']  # unit MWh/t material
...
# Efficiency changes due to electrification
eff_elec = s_ued['Paper: Paper machine - Electricity'] / \
    s_fec['Paper: Paper machine - Electricity']
df.loc['elec', sector] += s_ued['Paper: Paper machine - Steam use'] / eff_elec

eff_elec = s_ued['Paper: Product finishing - Electricity'] / \
    s_fec['Paper: Product finishing - Electricity']
...
df.loc[sources, sector] = df.loc[sources, sector] * conv_factor / \
    s_out['Physical output (index)']  # unit MWh/t material
...
sources = ['elec', 'biomass', 'methane', 'hydrogen', 'heat', 'naphtha']
df.loc[sources, sector] = df.loc[sources, sector] * conv_factor / \
    s_out['Alumina production (kt)']  # unit MWh/t material
...
# Efficiency changes due to electrification
eff_elec = s_ued['Aluminium processing - Electric'] / \
    s_fec['Aluminium processing - Electric']
df.loc['elec',
       sector] += s_ued['Aluminium processing  (metallurgy e.g. cast house, reheating)'] / eff_elec

# Efficiency changes due to electrification
eff_elec = s_ued['Aluminium finishing - Electric'] / \
    s_fec['Aluminium finishing - Electric']
...
df.loc['process emission', sector] = s_emi['Process emissions'] / \
    s_out['Aluminium - primary production']  # unit tCO2/t material

# final energy consumption per t
sources = ['elec', 'biomass', 'methane', 'hydrogen', 'heat', 'naphtha']
df.loc[sources, sector] = df.loc[sources, sector] * conv_factor / \
    s_out['Aluminium - primary production']  # unit MWh/t material
...
# Efficiency changes due to electrification
eff_elec = s_ued['Secondary aluminium - Electric'] / \
    s_fec['Secondary aluminium - Electric']
df.loc['elec',
       sector] += s_ued['Secondary aluminium (incl. pre-treatment, remelting)'] / eff_elec
...
sources = ['elec', 'biomass', 'methane', 'hydrogen', 'heat', 'naphtha']
df.loc[sources, sector] = df.loc[sources, sector] * conv_factor / \
    s_out['Aluminium - secondary production']  # unit MWh/t material
# 1 ktoe = 11630 MWh

# primary route is divided into 50% remains as today and 50% is
# transformed into secondary route
df.loc[sources,
       'Aluminium - primary production'] = (1 - snakemake.config["industry"]["Al_to_scrap"]) * df.loc[sources,
                                                                                                      'Aluminium - primary production'] + snakemake.config["industry"]["Al_to_scrap"] * df.loc[sources,
                                                                                                                                                                                               'Aluminium - secondary production']
df.loc['process emission',
       'Aluminium - primary production'] = (1 - snakemake.config["industry"]["Al_to_scrap"]) * df.loc['process emission',
...
# Efficiency changes due to electrification
eff_elec = s_ued['Metal production - Electric'] / \
    s_fec['Metal production - Electric']
df.loc['elec', sector] += s_ued['Other Metals: production'] / eff_elec

# Efficiency changes due to electrification
eff_elec = s_ued['Metal processing - Electric'] / \
    s_fec['Metal processing - Electric']
df.loc['elec',
       sector] += s_ued['Metal processing  (metallurgy e.g. cast house, reheating)'] / eff_elec

# Efficiency changes due to electrification
eff_elec = s_ued['Metal finishing - Electric'] / \
    s_fec['Metal finishing - Electric']
...
df.loc['process emission', sector] = s_emi['Process emissions'] / \
    s_out['Other non-ferrous metals (kt lead eq.)']  # unit tCO2/t material

# final energy consumption per t
sources = ['elec', 'biomass', 'methane', 'hydrogen', 'heat', 'naphtha']
df.loc[sources, sector] = df.loc[sources, sector] * conv_factor / \
    s_out['Other non-ferrous metals (kt lead eq.)']  # unit MWh/t material
...
# Efficiency changes due to electrification
eff_elec = s_ued['Trans. Eq.: Electric Foundries'] / \
    s_fec['Trans. Eq.: Electric Foundries']
df.loc['elec', sector] += s_ued['Trans. Eq.: Foundries'] / eff_elec

# Efficiency changes due to electrification
eff_elec = s_ued['Trans. Eq.: Electric connection'] / \
    s_fec['Trans. Eq.: Electric connection']
df.loc['elec', sector] += s_ued['Trans. Eq.: Connection techniques'] / eff_elec

# Efficiency changes due to electrification
eff_elec = s_ued['Trans. Eq.: Heat treatment - Electric'] / \
...
# Efficiency changes due to electrification
eff_elec = s_ued['Mach. Eq.: Electric Foundries'] / \
    s_fec['Mach. Eq.: Electric Foundries']
df.loc['elec', sector] += s_ued['Mach. Eq.: Foundries'] / eff_elec

# Efficiency changes due to electrification
eff_elec = s_ued['Mach. Eq.: Electric connection'] / \
    s_fec['Mach. Eq.: Electric connection']
df.loc['elec', sector] += s_ued['Mach. Eq.: Connection techniques'] / eff_elec

# Efficiency changes due to electrification
eff_elec = s_ued['Mach. Eq.: Heat treatment - Electric'] / \
...
# Efficiency changes due to electrification
eff_elec = s_ued['Textiles: Electric drying'] / \
    s_fec['Textiles: Electric drying']
...
# Efficiency changes due to electrification
eff_elec = s_ued['Other Industrial sectors: Electric processing'] / \
...
df.loc['elec',
       sector] += s_fec['Other Industrial sectors: Diesel motors (incl. biofuels)']
