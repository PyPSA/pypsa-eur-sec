...
# data frame final energy consumption per country and source
countries_df = pd.DataFrame(columns=index)
...
dic_sec = {
    'Iron and steel': 'Iron & steel industry',
    ...
    for sector in sectors:
        if country in non_EU:
            if country == 'CH':
                ...
                excel_balances = pd.read_excel(
                    '{}/{}.XLSX'.format(
                        eb_base_dir,
                        dic_countries[country]),
                    sheet_name='2016',
                    index_col=2,
                    header=0,
                    skiprows=1,
                    squeeze=True)
                e_country = excel_balances.loc[dic_sec[sector],
                                               'Total all products']

            # energy consumption in the sector and EU28
            excel_sum_out = pd.read_excel(
                '{}/JRC-IDEES-2015_Industry_EU28.xlsx'.format(jrc_base_dir),
                sheet_name='Ind_Summary',
                index_col=0,
                header=0,
            ...
            for subsector in sect2sub[sector]:
                ...
                for ind in index:
                    # kton * MWh = GWh (# kton * tCO2 = ktCO2)
                    countries_df.loc[country,
                                     ind] += float(output * df.loc[ind, subsector])
            ...
            for subsector in sect2sub[sector]:
                ...
                for ind in index:
                    # kton * MWh = GWh (# kton * tCO2 = ktCO2)
                    countries_df.loc[country, ind] += output * \
                        df.loc[ind, subsector]
...
for country in countries:
    if country in non_EU:
        if country == 'CH':
            countries_df.loc[country,
                             'current electricity'] = dic_Switzerland['current electricity'] * tj_to_ktoe * ktoe_to_twh
        else:
            excel_balances = pd.read_excel(
                '{}/{}.XLSX'.format(
                    eb_base_dir,
            ...
            countries_df.loc[country,
                             'current electricity'] = excel_balances.loc['Industry',
                                                                         'Electricity'] * ktoe_to_twh
        ...
        countries_df.loc[country,
                         'current electricity'] = s_out['Electricity'] * ktoe_to_twh
